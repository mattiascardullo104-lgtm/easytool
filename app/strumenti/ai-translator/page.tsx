...
export default function AITranslator() {
  ...
  const translate = async () => {
    if (!text.trim()) {
      setError("Write some text to translate.");
      return;
    }
    setError("");
    setResult("");
    ...
    try {
      const res = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          prompt: text,
          system: `Translate the following text into ${language}. Output only the translation.`,
        }),
      });
  ...
  return (
    <main className="min-h-screen bg-[var(--bg-base)] px-6 py-12">
      <div className="max-w-2xl mx-auto">
        ...
        <p className="font-tool text-[11px] text-[var(--text-muted)] text-center mt-6">
          AI translation, free. Great for quick drafts.
        </p>
      </div>
    </main>
  );
